...
FUNC_DECL float inverseSF(float3 p, float n)
{
    ...
    float k = max(2.f, floor(
        log(n * PI * sqrt(5.f) * (1.f - cosTheta*cosTheta))
            / log(PHI*PHI)));
...
FUNC_DECL float inverseHSF(float3 p, float n)
{
    ...
    float k = max(2.f, floor(
        log(2.f * n * PI * sqrt(5.f) * saturate(1.f - cosTheta*cosTheta))
            / log(PHI*PHI)));
    ...
    float2x2 invB = inverse(B);
    float2 c = floor(
        mul(invB, float2(phi, cosTheta - (1-0.5/n))));

    float d = INFINITY, j = 0;
    for (uint s = 0; s < 4; ++s) {
        float cosTheta =
            dot(B[1], float2(s%2, s/2) + c) + (1-0.5/n);
        if (cosTheta < 0)
            cosTheta = dot(B[1], c + (float2(s%2, s/2)*2.f - 1.f)) + (1-0.5/n);
