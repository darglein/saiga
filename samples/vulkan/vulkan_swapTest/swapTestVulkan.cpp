...
void swapTestVulkan()
{
    ...
    for (auto i : images)
    {
        ...
        colorAttachmentView.components = {vk::ComponentSwizzle::eIdentity, vk::ComponentSwizzle::eIdentity,
    ...
    std::vector<vk::CommandBuffer> commandBuffers;
    {
        ...
        assert(commandPool);
        vk::CommandBufferAllocateInfo cmdBufAllocateInfo(commandPool, vk::CommandBufferLevel::ePrimary, images.size());
    ...
    while (true)
    {
        ...
        if (duration > std::chrono::seconds(time)) break;
    }
    std::cout << "Rendered " << count << " frames in " << time << " seconds. -> " << count / double(time) << " fps."
              << std::endl;
