    ...
    MultiScaleUnet2dSlimImpl(MultiScaleUnet2dParams params) : params(params)
    {
      ...
      //  register_module("start", start.ptr());
        ...
        for (int i = 0; i < params.num_layers; ++i)
        {
            ...
            multistart[i]      = MultiStartBlock(multistart_in, multistart_out, params.conv_block, params.norm_layer_down, params.pooling,
                                           params.activation);
        ...
        //multistart[params.num_layers - 1] = MultiStartBlock(params.num_input_channels, filters[params.num_layers - 1], params.conv_block, 
...
{
   public:
    UpsampleUltraBlockImpl(int in_channels, int out_channels, int num_input_channels, std::string conv_block, std::string upsample_mode = "deconv",
                      std::string norm_str = "id", std::string activation = "id")
    {
        SAIGA_ASSERT(in_channels > 0);
        SAIGA_ASSERT(out_channels > 0);
        ...
        // conv = GatedBlock(out_channels * 2, out_channels, 3, 1, 1, norm_str);
        conv2 = UnetBlockFromString(conv_block, out_channels + num_input_channels, out_channels, 3, 1, 1, norm_str, activation);
...
class MultiScaleUnet2dUltraSlimImpl : public torch::nn::Module
{
   public:
    MultiScaleUnet2dUltraSlimImpl(MultiScaleUnet2dParams params) : params(params)
    {
      ...
      //  start      = MultiStartBlock(params.num_input_channels, filters[params.num_layers-1], params.conv_block, params.norm_layer_down, params.pooling,
       //                             params.activation);
       // register_module("start", start.ptr());
        ...
        //for (int i = params.num_layers - 1; i >= 1; --i)
        {
            ...
            int up_in  = filters[i + 1];
            int up_out = filters[i];
            up[i]      = UpsampleUltraBlock(up_in, up_out, params.num_input_channels, params.conv_block_up, params.upsample_mode, params.norm_layer_up,
                                       params.activation);
            register_module("up" + std::to_string(i + 1), up[i]);
        }
        //multistart[params.num_layers - 1] = MultiStartBlock(params.num_input_channels, filters[params.num_layers - 1], params.conv_block, 
...
//        d[0] = multistart[0].forward<std::pair<torch::Tensor, torch::Tensor>>(inputs[0], masks[0]);
      ...
      //  d[params.num_layers-1] = start.forward<std::pair<torch::Tensor, torch::Tensor>>(inputs[params.num_layers-1], masks[params.num_layers-1]);
