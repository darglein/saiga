...
class CostBAStereoAnalytic : public ceres::SizedCostFunction<2, 7, 3>
{
   public:
    using T = double;
    ...
    CostBAStereoAnalytic(const Intrinsics4& intr, const Eigen::Vector2d& observed, double weight = 1)
        : intr(intr), observed(observed), weight(weight)
    {
    }
