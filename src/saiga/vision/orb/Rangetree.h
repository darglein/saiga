...
class RangeTree
{
   public:
    RangeTree(u32 estimatedTotal = 1000, u32 estimatedResult = 100)
        : nodepool(NULL), ptypool(NULL), mainreserve(estimatedTotal), resultreserve(estimatedResult), init(false)
    {
    ...
    u32 count(point xmin, point xmax, point ymin, point ymax) const
    {
        ...
        if (xmax < xmin) pswap(xmax, xmin);
        if (ymax < ymin) pswap(ymax, ymin);
    ...
    void search(data** const arr, u32& arrsize, point xmin, point xmax, point ymin, point ymax) const
    {
        if (!init) return;
    ...
    static const char* version() { return "librangetree 1.3.1"; }
   ...
   private:
    ...
    u32 binarynext(const pty* const arr, const u32 ycount, const point goal) const
    {
        ...
        if (arr[t].y == goal)
        {
            while (t && arr[t - 1].y == goal) t--;
            return t;
        }

        if (arr[t].y < goal) return t + 1;
    ...
    u32 binarynextx(const std::vector<ptx>& arr, const point goal) const
    {
        ...
        if (arr[t].x == goal)
        {
            while (t && arr[t - 1].x == goal) t--;
            return t;
        }

        if (arr[t].x < goal) return t + 1;
    ...
    void build()
    {
        ...
        if (median != start.max) start.right = build(median + 1, start.max);
    ...
    void nuke()
    {
        ...
        if (!isptypooled(start.ypoints)) delete start.ypoints;
        // free(start.ypoints);
    }
    ...
    void nuke(node* const n)
    {
        ...
        if (!isptypooled(n->ypoints)) delete n->ypoints;
        // free(n->ypoints);


        if (!isnodepooled(n)) delete n;
    }

    void findnodes(const node* const n, const point xmin, const point xmax, const node* list[], u32& listcur) const
    {
        if (!n) return;

        // Fast outs
        if (xmin > n->max) return;
        if (xmax < n->min) return;
        if (!n->ycount) return;
    ...
    void mergekids(pty*& arr, u32& ycount, node* const __restrict__ left, node* const __restrict__ right) const
    {
        u32 l, r;
        ...
        for (l = 0, r = 0; l < lmax || r < rmax;)
        {
            ...
            if (l == lmax)
            {
                memcpy(&arr[cur], &right->ypoints[r], (rmax - r) * sizeof(pty));
                break;
            }
            else if (r == rmax)
            {
                memcpy(&arr[cur], &left->ypoints[l], (lmax - l) * sizeof(pty));
                break;
            }
            else
            {
    ...
    {
        char* ptr;
        asprintf(&ptr, "%u-%u, %u point%s", n->min, n->max, n->ycount, n->ycount > 1 ? "s" : "");
        return ptr;
    }

    void print(const node* const n)
    {
        printf("Node: max min %u %u, %u points\n", n->max, n->min, n->ycount);
    ...
    bool isnodepooled(const node* const n) const
    {
        ...
        if (n >= &nodepool[0] && n <= &nodepool[nodepoolcount - 1]) return true;
        return false;
    }

    bool isptypooled(const pty* const p) const
    {
        if (!ptypool) return false;

        if (p >= &ptypool[0] && p <= &ptypool[ptypoolcount - 1]) return true;
        return false;
    }
