...
// Businesses: you can support continued maintenance and development via support contracts or sponsoring, see
// docs/README. Individuals: you can support continued maintenance and development via donations or Patreon
...
/*
 ...
 http://mollyrocket.com/861 & more links docs/README.md.
 ...
 inputs from the rest of your application. In every cases you need to pass on the inputs to Dear ImGui. Refer to the FAQ
 ...
 IsMouseHoveringWindow(), IMGUI_ONCE_UPON_A_FRAME. Grep this log for details and new names.
 ...
 - 2019/01/06 (1.67) - renamed ImFontAtlas::GlyphRangesBuilder to ImFontGlyphRangesBuilder. Keep redirection typedef
 (will obsolete).
 ...
 ImGuiCond_FirstUseEver to ImGuiCond_FirstUseEver, ImGuiSetCond_Appearing to ImGuiCond_Appearing. Kept redirection enums
 (will obsolete).
 ...
 YourTextureIdentifier; } you now have more flexibility to load multiple TTF fonts and manage the texture buffer for
 internal needs. it is now recommended that you sample the font texture with bilinear interpolation. (1.30) - added
 texture identifier in ImDrawCmd passed to your render function (we can now render images). make sure to set
 io.Fonts->TexID. (1.30) - removed IO.PixelCenterOffset (unnecessary, can be handled in user projection matrix) (1.30) -
 removed ImGui::IsItemFocused() in favor of ImGui::IsItemActive() which handles all widgets
 ...
 value to ImTextureID / void*, and vice-versa. Because both end-points (user code and rendering function) are under your
 control, you know exactly what is stored inside the ImTextureID / void*. Examples:
 ...
 conversions. This way you'll be able to use your own types everywhere, e.g. passing glm::vec2 to ImGui functions
 instead of ImVec2.
 ...
 level.) (Read the 'misc/fonts/README.txt' file for more details about font loading.)
 ...
 characters the game need) ImVector<ImWchar> ranges; ImFontGlyphRangesBuilder builder; builder.AddText("Hello world");
 // Add a string (here "Hello world" contains 7 unique characters) builder.AddChar(0x7262); // Add a specific character
 ...
 A: - Being highly portable (bindings for several languages, frameworks, programming style, obscure or older
 platforms/compilers), and aiming for compatibility & performance suitable for every modern real-time game engines, dear
 ...
 more use cases.
    ...
    - Generally for most high-level types you should be able to access the underlying data type.
      You may write your own one-liner wrappers to facilitate user code (tip: add new functions in ImGui:: namespace
 from your code).
    - Dear ImGui applications often need to make intensive use of strings. It is expected that many of the strings you
 will pass to the API are raw literals (free in C/C++) or allocated in a manner that won't incur a large cost on your
 application. Please bear in mind that using std::string on applications with large amount of UI may incur
 unsatisfactory performances. Modern implementations of std::string often include small-string optimization (which is
 often a local buffer) but those are not configurable and not the same across implementations.
    - If you are finding your UI traversal cost to be too large, make sure your string usage is not leading to excessive
 amount of heap allocations. Consider using literals, statically sized buffers and your own helper functions. A common
 pattern is that you will need to build lots of strings on the fly, and their maximum length can be easily be scoped
 ahead. One possible implementation of a helper to facilitate printf-style building of strings:
 https://github.com/ocornut/Str This is a small helper where you can instance strings with configurable local buffers
 length. Many game engines will provide similar or better string helpers.
 ...
 A: - You can control Dear ImGui with a gamepad. Read about navigation in "Using gamepad/keyboard navigation controls".
      (short version: map gamepad inputs into the io.NavInputs[] array + set io.ConfigFlags |=
 ImGuiConfigFlags_NavEnableGamepad)
    - You can share your computer mouse seamlessly with your console/tablet/phone using Synergy
 (https://symless.com/synergy) This is the preferred solution for developer productivity. In particular, the
 "micro-synergy-client" repository (https://github.com/symless/micro-synergy-client) has simple and portable source code
 (uSynergy.c/.h) for a small embeddable client that you can use on any platform to connect to your host computer, based
 on the Synergy 1.x protocol. Make sure you download the Synergy 1 server on your computer. Console SDK also sometimes
 provide equivalent tooling or wrapper for Synergy-like protocols.
 ...
 A: - If you are experienced with Dear ImGui and C++, look at the github issues, look at the Wiki, read docs/TODO.txt
    ...
    - Businesses: convince your company to fund development via support contracts/sponsoring! This is among the most
 useful thing you can do for dear imgui.
    - Individuals: you can also become a Patron (http://www.patreon.com/imgui) or donate on PayPal! See README.
    - Disclose your usage of dear imgui via a dev blog post, a tweet, a screenshot, a mention somewhere etc.
      You may post screenshot or links in the gallery threads (github.com/ocornut/imgui/issues/1902). Visuals are ideal
...
#    pragma clang diagnostic \
        ignored "-Wold-style-cast"  // warning : use of old-style cast                              // yes, they are
                                    // more terse.
#    pragma clang diagnostic ignored \
...
#    pragma GCC diagnostic ignored \
        "-Wclass-memaccess"  // [__GNUC__ >= 8] warning: 'memset/memcpy' clearing/writing an object of type 'xxxx' with
...
static bool UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held,
...
ImGuiStyle::ImGuiStyle()
{
    ...
    DisplayWindowPadding = ImVec2(19, 19);  // Window position are clamped to be visible within the display area by at
...
{
    if (c > 0 && c < 0x10000) InputQueueCharacters.push_back((ImWchar)c);
}
                               ...
                               const ImVec2* text_size_if_known)
{
    ...
    if (text_size.x > pos_max.x - pos_min.x)
    {
        ...
        float text_size_clipped_x =
            font->CalcTextSizeA(font_size, text_width, 0.0f, text, text_end_full, &text_end_ellipsis).x;
...
// FIXME-NAV: The existence of SetNavID/SetNavIDWithRectRel/SetFocusID is incredibly messy and confusing and needs some
// explanation or refactoring.
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, ImGuiID id)
{
    ...
    if (is_tab_stop) window->DC.FocusCounterTab++;
    ...
    {
        if (window->DC.FocusCounterAll == g.FocusRequestCurrCounterAll) return true;
    ...
    if (g.IO.MouseClicked[0])
    {
        if (g.HoveredRootWindow != NULL)
        {
            ...
            if (g.IO.ConfigWindowsMoveFromTitleBarOnly && !(g.HoveredRootWindow->Flags & ImGuiWindowFlags_NoTitleBar))
                if (!g.HoveredRootWindow->TitleBarRect().Contains(g.IO.MouseClickedPos[0])) g.MovingWindow = NULL;
        }
...
void ImGui::UpdateMouseWheel()
{
    ...
    while ((window->Flags & ImGuiWindowFlags_ChildWindow) && (window->Flags & ImGuiWindowFlags_NoScrollWithMouse) &&
           !(window->Flags & ImGuiWindowFlags_NoScrollbar) && !(window->Flags & ImGuiWindowFlags_NoMouseInputs) &&
           window->ParentWindow)
...
void ImGui::NewFrame()
{
    ...
    if (g.TempInputTextId != 0 && g.ActiveId != g.TempInputTextId) g.TempInputTextId = 0;
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    //   Be mindful that the ImDrawList API doesn't filter vertices. Use the Metrics window to inspect draw list
    //   contents.
    ...
    //       for them.
    //   (B) Or handle 32-bits indices in your renderer back-end, and uncomment '#define ImDrawIdx unsigned int' line in
    //   imconfig.h.
...
bool ImGui::IsMouseClicked(int button, bool repeat)
{
    ...
    if (repeat && t > g.IO.KeyRepeatDelay)
    {
        ...
        int amount =
            CalcTypematicPressedRepeatAmount(t, t - g.IO.DeltaTime, g.IO.KeyRepeatDelay, g.IO.KeyRepeatRate * 0.5f);
...
static ImVec2 CalcContentSize(ImGuiWindow* window)
{
    ...
    ImVec2 sz;
    sz.x =
        (float)(int)((window->ContentSizeExplicit.x != 0.0f) ? window->ContentSizeExplicit.x
                                                             : window->DC.CursorMaxPos.x - window->DC.CursorStartPos.x);
    sz.y =
        (float)(int)((window->ContentSizeExplicit.y != 0.0f) ? window->ContentSizeExplicit.y
...
static bool ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held,
                                      int resize_grip_count, ImU32 resize_grip_col[4])
{
    ImGuiContext& g        = *GImGui;
    ...
    for (int border_n = 0; border_n < resize_border_count; border_n++)
    {
        ...
        if (held)
        {
            ...
            if (border_n == 0)
            {
                ...
                border_target.y =
                    (g.IO.MousePos.y - g.ActiveIdClickOffset.y + WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS);
            }  // Top
            if (border_n == 1)
            {
                border_posn = ImVec2(1, 0);
                border_target.x =
                    (g.IO.MousePos.x - g.ActiveIdClickOffset.x + WINDOWS_RESIZE_FROM_EDGES_HALF_THICKNESS);
            }  // Right
            if (border_n == 2)
            {
                                    ...
                                    int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        if (!(flags & ImGuiWindowFlags_NoBackground))
        {
            ...
            float alpha  = 1.0f;
            if (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasBgAlpha) alpha = g.NextWindowData.BgAlphaVal;
            if (alpha != 1.0f) bg_col = (bg_col & ~IM_COL32_A_MASK) | (IM_F32_TO_INT8_SAT(alpha) << IM_COL32_A_SHIFT);
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        window->InnerClipRect.Max.x = ImFloor(0.5f + window->InnerRect.Max.x -
                                              ImMax(ImFloor(window->WindowPadding.y * 0.5f), window->WindowBorderSize));
        ...
        // FIXME-OBSOLETE: window->ContentsRegionRect.Max is currently very misleading / partly faulty, but some
        ...
        // Title bar
        if (!(flags & ImGuiWindowFlags_NoTitleBar)) RenderWindowTitleBarContents(window, title_bar_rect, name, p_open);
    ...
    if (flags & ImGuiWindowFlags_ChildWindow)
    {
        ...
        if (parent_window && (parent_window->Collapsed || parent_window->Hidden)) window->HiddenFramesCanSkipItems = 1;
    }
                  ...
                  ImGuiWindowFlags flags)
{
    ...
    if (size_first_use.x != 0.0f || size_first_use.y != 0.0f) SetNextWindowSize(size_first_use, ImGuiCond_FirstUseEver);

    // Old API feature: override the window background alpha with a parameter.
    if (bg_alpha_override >= 0.0f) SetNextWindowBgAlpha(bg_alpha_override);
...
// programmaticaly. If you want a window to never be focused, you may use the e.g. NoInputs flag.
...
void ImGui::EndGroup()
{
    ...
    // widgets. Also if you grep for LastItemId you'll notice it is only used in that context. (The tests not
    // symmetrical because ActiveIdIsAlive is an ID itself, in order to be able to handle ActiveId being overwritten
    // during the frame.)
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window, bool restore_focus_to_window_under_popup)
{
    ...
    if (ref_window)
    {
        ...
        for (; popup_count_to_keep < g.OpenPopupStack.Size; popup_count_to_keep++)
        {
            ...
            // Trim the stack when popups are not direct descendant of the reference window (the reference window is
            // often the NavWindow)
...
void ImGui::ClosePopupToLevel(int remaining, bool restore_focus_to_window_under_popup)
{
    ...
    if (restore_focus_to_window_under_popup)
    {
        ...
        {
            if (g.NavLayer == 0 && focus_window) focus_window = NavRestoreLastChildNavWindow(focus_window);
            FocusWindow(focus_window);
        }
    }
}
...
static ImVec2 ImGui::NavCalcPreferredRefPos()
{
    ...
    else
    {
        ...
        return ImFloor(
            ImClamp(pos, visible_rect.Min,
                    visible_rect.Max));  // ImFloor() is important because non-integer mouse position application in
...
#define NAV_MAP_KEY(_KEY, _NAV_INPUT)                              \
    ...
    {                                                              \
        ...
        if (g.IO.KeyAlt && !g.IO.KeyCtrl)  // AltGR is Alt+Ctrl, also even on keyboards without AltGR we don't want
                                           // Alt+Ctrl to open menu.
...
#undef NAV_MAP_KEY
    ...
    if (IsNavInputPressed(ImGuiNavInput_Cancel, ImGuiInputReadMode_Pressed))
    {
        if (g.ActiveId != 0)
        {
            if (!(g.ActiveIdBlockNavInputFlags & (1 << ImGuiNavInput_Cancel))) ClearActiveID();
        }
...
static void ImGui::NavUpdateWindowing()
{
    ...
    if (IsNavInputPressed(ImGuiNavInput_KeyMenu_, ImGuiInputReadMode_Pressed)) g.NavWindowingToggleLayer = true;
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    struct Funcs
    {
        ...
        {
            if (rect_type == RT_OuterRect)
            {
                return window->Rect();
            }
            else if (rect_type == RT_OuterRectClipped)
            {
                return window->OuterRectClipped;
            }
            else if (rect_type == RT_InnerRect)
            {
                return window->InnerRect;
            }
            else if (rect_type == RT_InnerClipRect)
            {
                return window->InnerClipRect;
            }
            else if (rect_type == RT_WorkRect)
            {
                return window->WorkRect;
            }
            else if (rect_type == RT_Contents)
            {
                ImVec2 min = window->InnerRect.Min - window->Scroll + window->WindowPadding;
                return ImRect(min, min + window->ContentSize);
            }
            else if (rect_type == RT_ContentsRegionRect)
            {
        ...
        {
            // Standalone tab bars (not associated to docking/windows functionality) currently hold no discernible
            // strings.
            char buf[256];
            char* p             = buf;
    ...
    {
        for (int n = 0; n < g.TabBars.Data.Size; n++) Funcs::NodeTabBar(g.TabBars.GetByIndex(n));
        ImGui::TreePop();
    }
    ...
    if (ImGui::TreeNode("Tools"))
    {
        ...
        const char* rects_names[RT_Count] = {"OuterRect", "OuterRectClipped", "InnerRect",         "InnerClipRect",
                                             "WorkRect",  "Contents",         "ContentsRegionRect"};
        ...
        if (show_windows_rects && g.NavWindow)
        {
            ...
            for (int n = 0; n < RT_Count; n++)
            {
                ...
                ImGui::Text("(%6.1f,%6.1f) (%6.1f,%6.1f) Size (%6.1f,%6.1f) %s", r.Min.x, r.Min.y, r.Max.x, r.Max.y,
                            r.GetWidth(), r.GetHeight(), rects_names[n]);
            }
            ImGui::Unindent();
        }
