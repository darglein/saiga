...
//
template <class Scalar>
Vector3<Scalar> finiteDifferenceRotationalVelocity(std::function<SO3<Scalar>(Scalar)> const& foo_R_bar, Scalar t,
                                                   Scalar h = Constants<Scalar>::epsilon())
{
    ...
    Matrix3<Scalar> dR_dt_in_frame_foo =
        curveNumDiff([&foo_R_bar](Scalar t0) -> Matrix3<Scalar> { return foo_R_bar(t0).matrix(); }, t, h);
    // velocity tensor
...
//
template <class Scalar>
Vector3<Scalar> finiteDifferenceRotationalVelocity(std::function<SE3<Scalar>(Scalar)> const& foo_T_bar, Scalar t,
                                                   Scalar h = Constants<Scalar>::epsilon())
{
