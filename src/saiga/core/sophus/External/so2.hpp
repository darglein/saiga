...
template <class Scalar_, int Options>
struct traits<Map<Sophus::SO2<Scalar_>, Options>> : traits<Sophus::SO2<Scalar_, Options>>
{
    using Scalar      = Scalar_;
...
template <class Scalar_, int Options>
struct traits<Map<Sophus::SO2<Scalar_> const, Options>> : traits<Sophus::SO2<Scalar_, Options> const>
{
    using Scalar      = Scalar_;
...
class SO2Base
{
   public:
    ...
    ///
    SOPHUS_FUNC SO2<Scalar> inverse() const { return SO2<Scalar>(unit_complex().x(), -unit_complex().y()); }
    ...
    {
        using std::sqrt;
        Scalar length = sqrt(unit_complex().x() * unit_complex().x() + unit_complex().y() * unit_complex().y());
        SOPHUS_ENSURE(length >= Constants<Scalar>::epsilon(), "Complex number should not be close to zero!");
    ...
    SOPHUS_FUNC PointProduct<PointDerived> operator*(Eigen::MatrixBase<PointDerived> const& p) const
    {
        Scalar const& real = unit_complex().x();
        Scalar const& imag = unit_complex().y();
        return PointProduct<PointDerived>(real * p[0] - imag * p[1], imag * p[0] + real * p[1]);
    }
    ...
    SOPHUS_FUNC HomogeneousPointProduct<HPointDerived> operator*(Eigen::MatrixBase<HPointDerived> const& p) const
    {
        Scalar const& real = unit_complex().x();
        Scalar const& imag = unit_complex().y();
        return HomogeneousPointProduct<HPointDerived>(real * p[0] - imag * p[1], imag * p[0] + real * p[1], p[2]);
    }
    ...
    {
        return Matrix<Scalar, num_parameters, DoF>(-unit_complex()[1], unit_complex()[0]);
    }
    ...
    ///
    SOPHUS_FUNC Sophus::Vector<Scalar, num_parameters> params() const { return unit_complex(); }
    ...
    ///
    SOPHUS_FUNC
    ComplexT const& unit_complex() const { return static_cast<Derived const*>(this)->unit_complex(); }
   ...
   private:
    ...
    ///
    SOPHUS_FUNC
    ComplexT& unit_complex_nonconst() { return static_cast<Derived*>(this)->unit_complex_nonconst(); }
};
...
class SO2 : public SO2Base<SO2<Scalar_, Options>>
{
   public:
    ...
    ///
    template <class OtherDerived>
    SOPHUS_FUNC SO2(SO2Base<OtherDerived> const& other) : unit_complex_(other.unit_complex())
    {
    }
    ...
    SOPHUS_FUNC explicit SO2(Transformation const& R)
        : unit_complex_(Scalar(0.5) * (R(0, 0) + R(1, 1)), Scalar(0.5) * (R(1, 0) - R(0, 1)))
    {
    ...
    ///
    SOPHUS_FUNC SO2(Scalar const& real, Scalar const& imag) : unit_complex_(real, imag) { Base::normalize(); }
    ...
    ///
    template <class D>
    SOPHUS_FUNC explicit SO2(Eigen::MatrixBase<D> const& complex) : unit_complex_(complex)
    {
    ...
    ///
    SOPHUS_FUNC explicit SO2(Scalar theta) { unit_complex_nonconst() = SO2<Scalar>::exp(theta).unit_complex(); }

    /// Accessor of unit complex number
    ///
    SOPHUS_FUNC ComplexMember const& unit_complex() const { return unit_complex_; }
    ...
    ///
    SOPHUS_FUNC static Sophus::Matrix<Scalar, num_parameters, DoF> Dx_exp_x(Tangent const& theta)
    {
        using std::cos;
        using std::sin;
    ...
    ///
    SOPHUS_FUNC static Transformation Dxi_exp_x_matrix_at_0(int) { return generator(); }
    ...
    ///
    SOPHUS_FUNC static Tangent lieBracket(Tangent const&, Tangent const&) { return Scalar(0); }
    ...
    static SO2 sampleUniform(UniformRandomBitGenerator& generator)
    {
        ...
        std::uniform_real_distribution<Scalar> uniform(-Constants<Scalar>::pi(), Constants<Scalar>::pi());
...
template <class Scalar_, int Options>
class Map<Sophus::SO2<Scalar_>, Options> : public Sophus::SO2Base<Map<Sophus::SO2<Scalar_>, Options>>
{
   public:
    ...
    ///
    SOPHUS_FUNC
    Map<Sophus::Vector2<Scalar>, Options> const& unit_complex() const { return unit_complex_; }

   protected:
    /// Mutator of unit_complex is protected to ensure class invariant.
    ///
    SOPHUS_FUNC
    Map<Sophus::Vector2<Scalar>, Options>& unit_complex_nonconst() { return unit_complex_; }
...
/// complex number / tuple).
template <class Scalar_, int Options>
class Map<Sophus::SO2<Scalar_> const, Options> : public Sophus::SO2Base<Map<Sophus::SO2<Scalar_> const, Options>>
{
   public:
    ...
    /// Accessor of unit complex number.
    ///
    SOPHUS_FUNC Map<Sophus::Vector2<Scalar> const, Options> const& unit_complex() const { return unit_complex_; }
