...
template <class Scalar_, int Options>
struct traits<Map<Sophus::SE2<Scalar_>, Options>> : traits<Sophus::SE2<Scalar_, Options>>
{
    using Scalar          = Scalar_;
...
template <class Scalar_, int Options>
struct traits<Map<Sophus::SE2<Scalar_> const, Options>> : traits<Sophus::SE2<Scalar_, Options> const>
{
    using Scalar          = Scalar_;
...
class SE2Base
{
   public:
    ...
    {
        return SE2<NewScalarType>(so2().template cast<NewScalarType>(), translation().template cast<NewScalarType>());
    }
    ...
    SOPHUS_FUNC Tangent log() const
    {
        ...
        {
            halftheta_by_tan_of_halftheta = Scalar(1.) - Scalar(1. / 12) * theta * theta;
        }
        else
        {
    ...
    SOPHUS_FUNC Transformation matrix() const
    {
        ...
        homogenious_matrix.row(2)                         = Matrix<Scalar, 1, 3>(Scalar(0), Scalar(0), Scalar(1));
    ...
    {
        return SE2Product<OtherDerived>(so2() * other.so2(), translation() + so2() * other.translation());
    }
    ...
    {
        const PointProduct<HPointDerived> tp = so2() * p.template head<2>() + p(2) * translation();
    ...
    ///
    SOPHUS_FUNC Line operator*(Line const& l) const { return Line((*this) * l.origin(), so2() * l.direction()); }
    ...
    ///
    SOPHUS_FUNC Matrix<Scalar, 2, 2> rotationMatrix() const { return so2().matrix(); }

    /// Takes in complex number, and normalizes it.
    ///
    /// Precondition: The complex number must not be close to zero.
    ///
    SOPHUS_FUNC void setComplex(Sophus::Vector2<Scalar> const& complex) { return so2().setComplex(complex); }
    ...
    SOPHUS_FUNC void setRotationMatrix(Matrix<Scalar, 2, 2> const& R)
    {
        ...
        typename SO2Type::ComplexT const complex(Scalar(0.5) * (R(0, 0) + R(1, 1)), Scalar(0.5) * (R(1, 0) - R(0, 1)));
    ...
    ///
    SOPHUS_FUNC
    SO2Type const& so2() const { return static_cast<Derived const*>(this)->so2(); }

    /// Mutator of translation vector.
    ///
    SOPHUS_FUNC
    TranslationType& translation() { return static_cast<Derived*>(this)->translation(); }

    /// Accessor of translation vector
    ///
    SOPHUS_FUNC
    TranslationType const& translation() const { return static_cast<Derived const*>(this)->translation(); }

    /// Accessor of unit complex number.
    ///
    SOPHUS_FUNC
    typename Eigen::internal::traits<Derived>::SO2Type::ComplexT const& unit_complex() const
    {
...
class SE2 : public SE2Base<SE2<Scalar_, Options>>
{
   public:
    ...
    ///
    template <class OtherDerived>
    SOPHUS_FUNC SE2(SE2Base<OtherDerived> const& other) : so2_(other.so2()), translation_(other.translation())
    {
    ...
    ///
    SOPHUS_FUNC SE2(Scalar const& theta, Point const& translation) : so2_(theta), translation_(translation) {}

    /// Constructor from complex number and translation vector
    ///
    /// Precondition: ``complex`` must not be close to zero.
    SOPHUS_FUNC SE2(Vector2<Scalar> const& complex, Point const& translation) : so2_(complex), translation_(translation)
    {
    }
    ...
    SOPHUS_FUNC explicit SE2(Transformation const& T)
        : so2_(T.template topLeftCorner<2, 2>().eval()), translation_(T.template block<2, 1>(0, 2))
    {
    }
    ...
    /// Accessor of translation vector
    ///
    SOPHUS_FUNC TranslationMember const& translation() const { return translation_; }

    /// Returns derivative of exp(x) wrt. x.
    ///
    SOPHUS_FUNC static Sophus::Matrix<Scalar, num_parameters, DoF> Dx_exp_x(Tangent const& upsilon_theta)
    {
        using std::abs;
        using std::cos;
        using std::pow;
      ...
      J << o, o, o, o, o, i, i, o, -Scalar(0.5) * upsilon[1], o, i,
        ...
        J(2, 0)        = c3;
        J(2, 1)        = -c5;
        J(2, 2)        = -c3 * upsilon[1] + c6 * upsilon[0] - c8 * upsilon[0] + c9 * upsilon[1];
        J(3, 0)        = c5;
        J(3, 1)        = c3;
        J(3, 2)        = c3 * upsilon[0] + c6 * upsilon[1] - c8 * upsilon[1] - c9 * upsilon[0];
        return J;
    }
    ...
    SOPHUS_FUNC static SE2<Scalar> exp(Tangent const& a)
    {
        ...
        if (abs(theta) < Constants<Scalar>::epsilon())
        {
            ...
            one_minus_cos_theta_by_theta = Scalar(0.5) * theta - Scalar(1. / 24.) * theta * theta_sq;
        }
        else
        {
            sin_theta_by_theta           = so2.unit_complex().y() / theta;
            one_minus_cos_theta_by_theta = (Scalar(1.) - so2.unit_complex().x()) / theta;
        }
        Vector2<Scalar> trans(sin_theta_by_theta * a[0] - one_minus_cos_theta_by_theta * a[1],
    ...
    {
        return SE2(SO2<Scalar>::fitToSO2(T.template block<2, 2>(0, 0)), T.template block<2, 1>(0, 2));
    }
    ...
    ///
    static SOPHUS_FUNC SE2 rot(Scalar const& x) { return SE2(SO2<Scalar>(x), Sophus::Vector2<Scalar>::Zero()); }
    ...
    static SE2 sampleUniform(UniformRandomBitGenerator& generator)
    {
        ...
        return SE2(SO2<Scalar>::sampleUniform(generator), Vector2<Scalar>(uniform(generator), uniform(generator)));
    }
    ...
    static SOPHUS_FUNC SE2 trans(Vector2<Scalar> const& xy) { return SE2(SO2<Scalar>(), xy); }

    /// Construct x-axis translation.
    ///
    static SOPHUS_FUNC SE2 transX(Scalar const& x) { return SE2::trans(x, Scalar(0)); }

    /// Construct y-axis translation.
    ///
    static SOPHUS_FUNC SE2 transY(Scalar const& y) { return SE2::trans(Scalar(0), y); }
    ...
    {
        SOPHUS_ENSURE(Omega.row(2).template lpNorm<1>() < Constants<Scalar>::epsilon(), "Omega: \n%", Omega);
...
{
    static_assert(std::is_standard_layout<SE2>::value, "Assume standard layout for the use of offsetof check below.");
    static_assert(offsetof(SE2, so2_) + sizeof(Scalar) * SO2<Scalar>::num_parameters == offsetof(SE2, translation_),
...
template <class Scalar_, int Options>
class Map<Sophus::SE2<Scalar_>, Options> : public Sophus::SE2Base<Map<Sophus::SE2<Scalar_>, Options>>
{
   public:
    ...
    SOPHUS_FUNC
    Map(Scalar* coeffs) : so2_(coeffs), translation_(coeffs + Sophus::SO2<Scalar>::num_parameters) {}
    ...
    /// Accessor of SO3
    ///
    SOPHUS_FUNC Map<Sophus::SO2<Scalar>, Options> const& so2() const { return so2_; }

    /// Mutator of translation vector
    ///
    SOPHUS_FUNC Map<Sophus::Vector2<Scalar>, Options>& translation() { return translation_; }

    /// Accessor of translation vector
    ///
    SOPHUS_FUNC Map<Sophus::Vector2<Scalar>, Options> const& translation() const { return translation_; }
...
/// Allows us to wrap SE2 objects around POD array.
template <class Scalar_, int Options>
class Map<Sophus::SE2<Scalar_> const, Options> : public Sophus::SE2Base<Map<Sophus::SE2<Scalar_> const, Options>>
{
   public:
    ...
    SOPHUS_FUNC Map(Scalar const* coeffs) : so2_(coeffs), translation_(coeffs + Sophus::SO2<Scalar>::num_parameters) {}

    /// Accessor of SO3
    ///
    SOPHUS_FUNC Map<Sophus::SO2<Scalar> const, Options> const& so2() const { return so2_; }

    /// Accessor of translation vector
    ///
    SOPHUS_FUNC Map<Sophus::Vector2<Scalar> const, Options> const& translation() const { return translation_; }
