...
template <class Scalar_, int Options>
struct traits<Map<Sophus::RxSO2<Scalar_>, Options>> : traits<Sophus::RxSO2<Scalar_, Options>>
{
    using Scalar      = Scalar_;
...
template <class Scalar_, int Options>
struct traits<Map<Sophus::RxSO2<Scalar_> const, Options>> : traits<Sophus::RxSO2<Scalar_, Options> const>
{
    using Scalar      = Scalar_;
...
class RxSO2Base
{
   public:
    ...
    SOPHUS_FUNC RxSO2<Scalar> inverse() const
    {
        ...
        return RxSO2<Scalar>(complex().x() / squared_scale, -complex().y() / squared_scale);
    }
    ...
    SOPHUS_FUNC RxSO2Product<OtherDerived> operator*(RxSO2Base<OtherDerived> const& other) const
    {
        ...
        typename RxSO2Product<OtherDerived>::ComplexType result_complex(lhs_real * rhs_real - lhs_imag * rhs_imag,
                                                                        lhs_real * rhs_imag + lhs_imag * rhs_real);

        const ResultT squared_scale = result_complex.squaredNorm();

        if (squared_scale < Constants<ResultT>::epsilon() * Constants<ResultT>::epsilon())
        {
    ...
    ///
    SOPHUS_FUNC Sophus::Vector<Scalar, num_parameters> params() const { return complex(); }
    ...
    {
        SOPHUS_ENSURE(z.squaredNorm() > Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon(),
    ...
    /// Accessor of complex.
    ///
    SOPHUS_FUNC ComplexType const& complex() const { return static_cast<Derived const*>(this)->complex(); }
    ...
    ///
    SOPHUS_FUNC void setRotationMatrix(Transformation const& R) { setSO2(SO2<Scalar>(R)); }
    ...
    {
        SOPHUS_ENSURE(isScaledOrthogonalAndPositive(sR), "sR must be scaled orthogonal:\n %", sR);
   ...
   protected:
    ...
    ///
    SOPHUS_FUNC ComplexType& complex_nonconst() { return static_cast<Derived*>(this)->complex_nonconst(); }
};
...
class RxSO2 : public RxSO2Base<RxSO2<Scalar_, Options>>
{
   public:
    ...
    ///
    template <class OtherDerived>
    SOPHUS_FUNC RxSO2(RxSO2Base<OtherDerived> const& other) : complex_(other.complex())
    {
    }
    ...
    /// determinant of ``s^2``.
    ///
    SOPHUS_FUNC explicit RxSO2(Transformation const& sR) { this->setScaledRotationMatrix(sR); }
    ...
    ///
    SOPHUS_FUNC RxSO2(Scalar const& scale, SO2<Scalar> const& so2) : RxSO2((scale * so2.unit_complex()).eval()) {}
    ...
    {
        SOPHUS_ENSURE(complex_.squaredNorm() >= Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon(),
                      "Scale factor must be greater-equal epsilon: % vs %", complex_.squaredNorm(),
    ...
    /// Precondition: complex number must not be close to zero.
    ///
    SOPHUS_FUNC explicit RxSO2(Scalar const& real, Scalar const& imag) : RxSO2(Vector2<Scalar>(real, imag)) {}
    ...
    static RxSO2 sampleUniform(UniformRandomBitGenerator& generator)
    {
        ...
        using std::exp2;
        return RxSO2(exp2(uniform(generator)), SO2<Scalar>::sampleUniform(generator));
    }
...
/// complex).
template <class Scalar_, int Options>
class Map<Sophus::RxSO2<Scalar_>, Options> : public Sophus::RxSO2Base<Map<Sophus::RxSO2<Scalar_>, Options>>
{
   ...
   public:
    ...
    ///
    SOPHUS_FUNC
    Map<Sophus::Vector2<Scalar>, Options> const& complex() const { return complex_; }

   protected:
    SOPHUS_FUNC Map<Sophus::Vector2<Scalar>, Options>& complex_nonconst() { return complex_; }
...
/// complex).
template <class Scalar_, int Options>
class Map<Sophus::RxSO2<Scalar_> const, Options> : public Sophus::RxSO2Base<Map<Sophus::RxSO2<Scalar_> const, Options>>
{
   public:
    ...
    ///
    SOPHUS_FUNC
    Map<Sophus::Vector2<Scalar> const, Options> const& complex() const { return complex_; }
