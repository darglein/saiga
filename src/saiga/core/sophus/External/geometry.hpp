...
{
    SOPHUS_ENSURE(normal_foo.squaredNorm() > Constants<T>::epsilon(), "%", normal_foo.transpose());
...
///
template <class T>
Matrix3<T> rotationFromNormal(Vector3<T> const& normal_foo, Vector3<T> xDirHint_foo = Vector3<T>(T(1), T(0), T(0)),
                              Vector3<T> yDirHint_foo = Vector3<T>(T(0), T(1), T(0)))
{
    SOPHUS_ENSURE(xDirHint_foo.dot(yDirHint_foo) < Constants<T>::epsilon(),
                  "xDirHint (%) and yDirHint (%) must be perpendicular.", xDirHint_foo.transpose(),
    ...
    SOPHUS_ENSURE(xDirHint_foo_sqr_length > Constants<T>::epsilon(), "%", xDirHint_foo.transpose());
    SOPHUS_ENSURE(yDirHint_foo_sqr_length > Constants<T>::epsilon(), "%", yDirHint_foo.transpose());
    SOPHUS_ENSURE(normal_foo_sqr_length > Constants<T>::epsilon(), "%", normal_foo.transpose());
    ...
    // sanity check
    SOPHUS_ENSURE(abs(det - T(1)) < Constants<T>::epsilon(), "Determinant of basis is not 1, but %. Basis is \n%\n",
                  det, basis_foo);
    return basis_foo;
}
