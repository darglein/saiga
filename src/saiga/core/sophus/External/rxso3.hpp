...
template <class Scalar_, int Options>
struct traits<Map<Sophus::RxSO3<Scalar_>, Options>> : traits<Sophus::RxSO3<Scalar_, Options>>
{
    using Scalar         = Scalar_;
...
template <class Scalar_, int Options>
struct traits<Map<Sophus::RxSO3<Scalar_> const, Options>> : traits<Sophus::RxSO3<Scalar_, Options> const>
{
    using Scalar         = Scalar_;
...
class RxSO3Base
{
   public:
    ...
    ///
    SOPHUS_FUNC Scalar const* data() const { return quaternion().coeffs().data(); }

    /// Returns group inverse.
    ///
    SOPHUS_FUNC RxSO3<Scalar> inverse() const { return RxSO3<Scalar>(quaternion().inverse()); }
    ...
    SOPHUS_FUNC RxSO3Product<OtherDerived> operator*(RxSO3Base<OtherDerived> const& other) const
    {
        ...
        typename RxSO3Product<OtherDerived>::QuaternionType result_quaternion(quaternion() * other.quaternion());
    ...
    SOPHUS_FUNC PointProduct<PointDerived> operator*(Eigen::MatrixBase<PointDerived> const& p) const
    {
        ...
        return scale * p + (quaternion().w() * two_vec_cross_p + quaternion().vec().cross(two_vec_cross_p));
    }
    ...
    {
        return Line((*this) * l.origin(), (*this) * l.direction() / quaternion().squaredNorm());
    }
    ...
    /// quaternion.
    ///
    SOPHUS_FUNC Sophus::Vector<Scalar, num_parameters> params() const { return quaternion().coeffs(); }
    ...
    {
        SOPHUS_ENSURE(quat.squaredNorm() > Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon(),
    ...
    ///
    SOPHUS_FUNC QuaternionType const& quaternion() const { return static_cast<Derived const*>(this)->quaternion(); }
    ...
    SOPHUS_FUNC void setScaledRotationMatrix(Transformation const& sR)
    {
        ...
        Scalar squared_scale      = Scalar(1. / 3.) * (squared_sR(0, 0) + squared_sR(1, 1) + squared_sR(2, 2));
        SOPHUS_ENSURE(squared_scale >= Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon(),
                      "Scale factor must be greater-equal epsilon.");
   ...
   protected:
    ...
    ///
    SOPHUS_FUNC QuaternionType& quaternion_nonconst() { return static_cast<Derived*>(this)->quaternion_nonconst(); }
};
...
class RxSO3 : public RxSO3Base<RxSO3<Scalar_, Options>>
{
   public:
    ...
    /// to 1.
    ///
    SOPHUS_FUNC RxSO3() : quaternion_(Scalar(1), Scalar(0), Scalar(0), Scalar(0)) {}
    ...
    ///
    template <class OtherDerived>
    SOPHUS_FUNC RxSO3(RxSO3Base<OtherDerived> const& other) : quaternion_(other.quaternion())
    {
    }
    ...
    /// determinant of ``s^3``.
    ///
    SOPHUS_FUNC explicit RxSO3(Transformation const& sR) { this->setScaledRotationMatrix(sR); }
    ...
    ///
    SOPHUS_FUNC RxSO3(Scalar const& scale, Transformation const& R) : quaternion_(R)
    {
        SOPHUS_ENSURE(scale >= Constants<Scalar>::epsilon(), "Scale factor must be greater-equal epsilon.");
        using std::sqrt;
    ...
    ///
    SOPHUS_FUNC RxSO3(Scalar const& scale, SO3<Scalar> const& so3) : quaternion_(so3.unit_quaternion())
    {
        SOPHUS_ENSURE(scale >= Constants<Scalar>::epsilon(), "Scale factor must be greater-equal epsilon.");
        using std::sqrt;
        quaternion_.coeffs() *= sqrt(scale);
    ...
    ///
    template <class D>
    SOPHUS_FUNC explicit RxSO3(Eigen::QuaternionBase<D> const& quat) : quaternion_(quat)
    {
    ...
    ///
    SOPHUS_FUNC static RxSO3<Scalar> expAndTheta(Tangent const& a, Scalar* theta)
    {
        ...
        Eigen::Quaternion<Scalar> quat = SO3<Scalar>::expAndTheta(omega, theta).unit_quaternion();
    ...
    static RxSO3 sampleUniform(UniformRandomBitGenerator& generator)
    {
        ...
        using std::exp2;
        return RxSO3(exp2(uniform(generator)), SO3<Scalar>::sampleUniform(generator));
    }
...
/// quaternion).
template <class Scalar_, int Options>
class Map<Sophus::RxSO3<Scalar_>, Options> : public Sophus::RxSO3Base<Map<Sophus::RxSO3<Scalar_>, Options>>
{
   public:
    ...
    ///
    SOPHUS_FUNC
    Map<Eigen::Quaternion<Scalar>, Options> const& quaternion() const { return quaternion_; }

   protected:
    SOPHUS_FUNC Map<Eigen::Quaternion<Scalar>, Options>& quaternion_nonconst() { return quaternion_; }
...
/// quaternion).
template <class Scalar_, int Options>
class Map<Sophus::RxSO3<Scalar_> const, Options> : public Sophus::RxSO3Base<Map<Sophus::RxSO3<Scalar_> const, Options>>
{
   public:
    ...
    ///
    SOPHUS_FUNC
    Map<Eigen::Quaternion<Scalar> const, Options> const& quaternion() const { return quaternion_; }
