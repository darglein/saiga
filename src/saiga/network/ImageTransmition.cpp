...
ImageTransmition::ImageTransmition(std::string host, uint32_t port)
    : socket(ios)
{
...
{
    return h.mn == magicNumber &&
            (int)h.width == img.width &&
...
void ImageTransmition::sendImage(const Image &img)
{
    ...
    for(size_t i = 0 ; i < h.numDataPackets; ++i)
    {
        ...
        size_t packetSize = glm::min(dataPacketSize,imageSize-offset);
