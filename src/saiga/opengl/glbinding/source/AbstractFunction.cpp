...
namespace glbinding
{
AbstractFunction::AbstractFunction(const char* _name) : m_name(_name) {}
...
bool AbstractFunction::isEnabled(const CallbackMask mask) const
{
    ...
    return (static_cast<callback_mask_t>(state().callbackMask()) & static_cast<callback_mask_t>(mask)) ==
           static_cast<callback_mask_t>(mask);
}
