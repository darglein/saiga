...
class GLBINDING_AUX_API ContextInfo
{
   public:
     ...
     *   'true' if all extensions required for the given feature are supported, else 'false'
     */
    static bool supported(const Version& version, std::set<gl::GLextension>& unsupportedExtensions,
                          std::set<AbstractFunction*>& unsupportedFunctions, bool resolve = false);
};
