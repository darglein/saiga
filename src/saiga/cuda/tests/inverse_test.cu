...
void inverseTest(){
    ...
    Saiga::CUDA::PerformanceTestHelper test(
                "Matrix inverse test. MatrixSize: "
...
//                Ainv = A.llt().solve(fixed_matrix_t::Identity());
...

    {
        ...
        float time;
        {
            ...
            invertMatrices<double,MatrixSize,BLOCK_SIZE> <<< Saiga::CUDA::getBlockCount(MatrixCount,BLOCK_SIZE),BLOCK_SIZE >>>(d_data,d_result);
        }
...

    {
        ...
        {
            Saiga::CUDA::CudaScopedTimer t(time);
            invertMatrices2<double,MatrixSize,BLOCK_SIZE> <<< Saiga::CUDA::getBlockCount(MatrixCount,BLOCK_SIZE),BLOCK_SIZE >>>(d_data,d_result);
        }
...

    {
        ...
        {
            Saiga::CUDA::CudaScopedTimer t(time);
            invertMatrices3<double,MatrixSize,BLOCK_SIZE> <<< Saiga::CUDA::getBlockCount(MatrixCount,BLOCK_SIZE),BLOCK_SIZE >>>(d_data,d_result);
        }
