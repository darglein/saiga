...
__global__ static void d_convolve3x3(ImageView<float> src, ImageView<float> dst)
{
...
template <unsigned int TILE_W, unsigned int TILE_H>
__global__ static void d_convolve3x3Shared(ImageView<float> src, ImageView<float> dst)
{
    const unsigned int tx = threadIdx.x;
    const unsigned int ty = threadIdx.y;
    const unsigned int t  = ty * TILE_W + tx;
...
template <unsigned int TILE_W, unsigned int TILE_H>
__global__ static void d_convolve3x3Shared2(ImageView<float> src, ImageView<float> dst)
{
    const unsigned int tx        = threadIdx.x;
    const unsigned int ty        = threadIdx.y;
    const unsigned int t         = ty * TILE_W + tx;
    ...
    // copy main data
    for (int i = 0; i < 2; ++i) sbuffer[ty + i * TILE_H + 1][tx + 1] = src.clampedRead(y + i * TILE_H, x);
...
__global__ static void d_convolve3x3Shared3(ImageView<float> src, ImageView<float> dst)
{
    ...
    // copy main data
    for (int i = 0; i < Y_ELEMENTS; ++i) sbuffer[ty + i * TILE_H][tx] = src.clampedRead(y + i * TILE_H, x);
...
template <unsigned int TILE_W, unsigned int TILE_H>
__global__ static void d_copySharedSync(ImageView<float> src, ImageView<float> dst)
{
    const unsigned int tx = threadIdx.x;
    const unsigned int ty = threadIdx.y;
    //    const unsigned int t = ty * TILE_W + tx;
...
template <unsigned int TILE_W, unsigned int TILE_H, unsigned int Y_ELEMENTS = 2>
__global__ static void d_copySharedSync2(ImageView<float> src, ImageView<float> dst)
{
    const unsigned int TILE_H2 = TILE_H * Y_ELEMENTS;
    ...
    for (int i = 0; i < Y_ELEMENTS; ++i) dst.clampedWrite(y + i * TILE_H, x, sbuffer[ty + i * TILE_H][tx]);
}
...
void convolutionTest3x3()
{
    ...
    CHECK_CUDA_ERROR(
        cudaMemcpyToSymbol(d_Kernel, h_kernel.data(), h_kernel.size() * sizeof(float), 0, cudaMemcpyHostToDevice));
...

    {
        auto st = Saiga::measureObject<Saiga::CUDA::CudaScopedTimer>(its, [&]() {
            ...
            const int TILE_H = 1;
            dim3 blocks(Saiga::iDivUp(w, TILE_W), Saiga::iDivUp(h, TILE_H), 1);
...

    {
        ...
        auto st = Saiga::measureObject<Saiga::CUDA::CudaScopedTimer>(its, [&]() {
            const int TILE_W = 32;
            const int TILE_H = 16;
            dim3 blocks(Saiga::iDivUp(w, TILE_W), Saiga::iDivUp(h, TILE_H * 2), 1);
            dim3 threads(TILE_W, TILE_H);
...

    {
        ...
        auto st = Saiga::measureObject<Saiga::CUDA::CudaScopedTimer>(its, [&]() {
            ...
            dim3 blocks(Saiga::iDivUp(w, TILE_W - 2), Saiga::iDivUp(h, TILE_H * Y_ELEMENTS - 2), 1);
            dim3 threads(TILE_W, TILE_H);
