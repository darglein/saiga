...
__device__ inline
T blockInclusiveScan(const CUDA::ThreadInfo<BLOCK_SIZE>& ti, T val, T* shared, T* sharedBlockPrefix) {
...
template<unsigned int BLOCK_SIZE, typename T>
__device__ inline
T blockInclusiveScan(T val, T* shared, T* sharedBlockPrefix) {
...
#pragma unroll
    for(int i = 0 ; i < TILES_PER_BLOCK; ++i){
        ...
        if(CHECK_BOUNDS && tileId >= numTiles)
            break;
