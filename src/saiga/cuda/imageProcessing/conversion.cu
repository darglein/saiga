...
template<int BLOCK_W, int BLOCK_H, int ROWS_PER_THREAD = 1>
__global__
static void d_convertRGBAtoRGB(ImageView<uchar4> src, ImageView<uchar3> dst, int h)
{
    const int tx = threadIdx.x;
    const int ty = threadIdx.y;
    int x = blockIdx.x*BLOCK_W + tx;
...
template<int BLOCK_W, int BLOCK_H, int ROWS_PER_THREAD = 1>
__global__
static void d_convertBGRtoRGBA(ImageView<uchar3> src, ImageView<uchar4> dst, int h, unsigned char alpha)
{
    const int tx = threadIdx.x;
    const int ty = threadIdx.y;
    int x = blockIdx.x*BLOCK_W + tx;
...
template<int BLOCK_W, int BLOCK_H, int ROWS_PER_THREAD = 1>
__global__
static void d_convertRGBAtoBGR(ImageView<uchar4> src, ImageView<uchar3> dst, int h)
{
    const int tx = threadIdx.x;
    const int ty = threadIdx.y;
    int x = blockIdx.x*BLOCK_W + tx;
