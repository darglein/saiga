...
namespace Vulkan
{
class SAIGA_VULKAN_API ProceduralSkybox : public Pipeline
{
   public:
    using VertexType = VertexNC;


    ~ProceduralSkybox() { destroy(); }
    void destroy();
     ...
     */
    void renderTexture(vk::CommandBuffer cmd, StaticDescriptorSet texture, vec2 position, vec2 size);
