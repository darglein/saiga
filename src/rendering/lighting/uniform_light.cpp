...
#include "saiga/rendering/lighting/uniform_light.h"
#include "saiga/geometry/clipping.h"
#include "saiga/geometry/obb.h"

namespace Saiga {

void UniformLightShader::checkUniforms(){
...
void UniformLightShader::uploadDirection(vec3 &direction){
    Shader::upload(location_direction,direction);
}

void UniformLightShader::uploadAmbientIntensity(float i)
{
    Shader::upload(location_ambientIntensity,i);
}


void UniformLightShader::uploadNumCascades(int n)
{
    Shader::upload(location_numCascades,n);
}

void UniformLightShader::uploadCascadeInterpolateRange(float r)
{
    Shader::upload(location_cascadeInterpolateRange,r);
}

void UniformLightShader::uploadSsaoTexture(std::shared_ptr<raw_Texture> texture)
{
...
void UniformLightShader::uploadDepthTextures(std::vector<std::shared_ptr<raw_Texture> > &textures){
...
void UniformLightShader::uploadViewToLightTransforms(std::vector<mat4> &transforms)
{
    Shader::upload(location_viewToLightTransforms,transforms.size(),transforms.data());
}

void UniformLightShader::uploadDepthCuts(std::vector<float> &depthCuts)
{
...
void UniformLight::createShadowMap(int resX, int resY, int numCascades){
...
void UniformLight::setDirection(const vec3 &dir){
...
void UniformLight::fitShadowToCamera(Camera *cam)
{
#if 0
    vec3 dir = -direction;
...
void UniformLight::fitNearPlaneToScene(AABB sceneBB)
{
...
void UniformLight::bindUniforms(UniformLightShader &shader, Camera *cam){
...
void UniformLight::bindCascade(int n){
...
void UniformLight::setDepthCutsRelative(const std::vector<float> &value)
{
...
std::vector<float> UniformLight::getDepthCutsRelative() const
{
