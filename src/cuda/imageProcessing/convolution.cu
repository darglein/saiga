...
void convolutionTest(){
    ...
    {
        dest = src;
        CHECK_CUDA_ERROR(cudaMemcpyToSymbol(d_Kernel, h_kernel.data(), kernel_size*sizeof(float)));
        float time;
        {
            ...
            //                cout << "lowpass threads: " << threads.x << " " << threads.y << " blocks: " << blocks.x << " " << blocks.y << endl;
...
//        CHECK_CUDA_ERROR(cudaMemcpyToSymbol(d_Kernel, h_kernel.data(), kernel_size*sizeof(float)));
        ...
        {
            Saiga::CUDA::CudaScopedTimer t(time);
            convolve<float,kernel_radius>(imgSrc,imgDst);
        }
