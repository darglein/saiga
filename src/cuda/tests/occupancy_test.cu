...
void occupancyTest2(){
    ...
    CUDA::PerformanceTestHelper pth("Occupancy for bandwidth limited kernels Vector size: " + std::to_string(sizeof(vectorT)), readWrites);
...
//            std::cout << blocksPerSM << " " << targetSharedMem << " = " << sharedMem << " / " << blocksPerSM << std::endl;
            ...
            const        size_t NUM_BLOCKS = CUDA::getBlockCount(N*sizeof(int)/sizeof(vectorT),THREADS_PER_BLOCK);
            {
                ...
                copy<THREADS_PER_BLOCK,vectorT><<<NUM_BLOCKS,THREADS_PER_BLOCK,targetSharedMem>>>(thrust::raw_pointer_cast(src.data()),thrust::raw_pointer_cast(dest.data()),N * sizeof(int));
            }
